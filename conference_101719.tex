...
\subsection{ロボティクスにおけるトポロジカルマップ}
ロボティクスにおける地図としての環境情報は，メトリックマップとトポロジカルマップの2種類に大別される．
...
このような表現は，計算効率や環境変化に対する頑健性の面で優れており，経路計画および自律移動において有効であり，人間の空間認知においてもトポロジカル構造に基づく認知地図の存在が示唆され，ロボティクスへの応用が進められている\cite{ref:The-Spatial-Semantic-Hierarchy}\cite{ref:Metric-Topological}\cite{ref:Navigation-in-hybrid-metric-topological-maps}．
...
Konolige ら\cite{ref:Navigation-in-hybrid-metric-topological-maps}もメトリック・トポロジカルマップを用いたナビゲーション手法を提案している．
...
初期の VPR は，SIFT や SURF などの local descriptor を bag of visual words\cite{ref:bag of visual words} や VLAD\cite{ref:VLAD} により固定長ベクトルに変換し，画像検索タスクに拡張する形式が取られていた．
...
\subsection{画像のパノラマシフトを用いた特徴抽出によるコサイン類似度の変化}
...
図\ref{fig:ex_sim_panorama_shift_outdoor}に屋外環境で取得した画像列に対するコサイン類似度の変化の例を示す．
...
\subsection{一貫性チェックを用いたループ検出によるトポロジカルマップの構築}
...
\begin{table}[t]
  ...
  \textbf{Method} & \textbf{Simple Th.} & \textbf{P. Shift \& Simple Th.} & \textbf{Ours}\\
  \midrule%[0.05em]  % デフォルト 0.05em（\lightrulewidth）
  P. Shift & \Cross & \Circle & \Circle \\
  Sp. Co. & \Cross & \Cross & \Circle \\
...
\begin{table}[t]
  ...
  F1 Score \textbf{$\uparrow$} & 5\% & 11\% & \textbf{47\%}\\
  \bottomrule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  \end{tabular}
\end{table}
