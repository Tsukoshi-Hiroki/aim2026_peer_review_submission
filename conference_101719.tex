...
類似度に対して閾値 \text{node\_sim\_th} を設定し，ノードの粗密を調整する．
具体的には，特徴ベクトル $\bm{v}_i$ を基準として，それ以降の特徴ベクトル $\bm{v}_j$ ($j>i$) との類似度が \text{node\_sim\_th} を上回った場合はノードは追加されず，下回った場合は$\bm{v}_j$が新たなノードとして追加される．
...
提案手法では，前稿\cite{ref:robosym_ogiwara}よりも \text{node\_sim\_th} を高く設定し，ノード間の距離を短くする．
...
ウィンドウには事前に決められた \text{window\_size} 個のノードが格納され，それらとクエリノードとの類似度を計算する．
...
まず，ウィンドウ内のノードのうち，$\bm{n}_q$ とループ検出に対する閾値 \text{loop\_sim\_th} 以上の類似度をもつ $\bm{n}_t$ をカウントする．
次に，一つのウィンドウに対するカウントがカウント用の閾値 \text{count\_th} を超えた場合，そのウィンドウをループ検出の候補ウィンドウとする．
...
スキップする範囲は，クエリノード $\bm{n}_q$ の前後 \text{neighbor\_node\_num} 分のノードとする．
