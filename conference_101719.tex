...
Kuipers らは認知地図（Cognitive Map）の概念をロボットの空間表現に取り入れ，環境を複数の抽象レベルで捉える Spatial Semantic Hierarchy（SSH）を提案した．\cite{ref:The-Spatial-Semantic-Hierarchy}

また Thrun は，グリッドマップの幾何学的な精密さと，トポロジカルマップの計算効率の高さを統合したハイブリッドな地図構築手法を提案した．
このメトリック・トポロジカルマップは,詳細な幾何情報と高次の抽象表現を両立させた地図表現を実現している．\cite{ref:Metric-Topological}

Konolige らもメトリック・トポロジカルマップを用いたナビゲーション手法を提案している．
小領域のメトリックな局所地図による局所的なナビゲーションとトポロジカルグラフによる大域的な経路計画を行うことで，詳細な幾何情報に常時依存する必要はないことを示唆している．\cite{ref:Navigation-in-hybrid-metric-topological-maps}

\subsection{Visual Place Recognition}
Visual Place Recognition（VPR）は，画像を用いて一度訪れた地点を認識する技術である．
初期の VPR は，SIFT や SURF などの local descriptor を bag-of-words\cite{ref:bag-of-words} や VLAD\cite{ref:VLAD} により固定長ベクトルに変換し，画像検索タスクに拡張する形式が取られていた．
...
前稿\cite{ref:robosym_ogiwara}では入力画像をパノラマ画像， VPR 手法として多種多様な環境で高い性能を発揮している AnyLoc\cite{ref:AnyLoc} を用いてトポロジカルマップの構築を行った．
しかし，再訪地点が交差・逆方向の場合においては画像を構成する物体の位置関係が異なることで，真に同一な地点においても視覚的類似度の低下が発生しループ検出ができない場合があった．
本稿では，パノラマシフトによるパノラマ画像の切れ目の変更を行うことで上記問題の解決を図る．
...
\begin{figure}[t]
  ...
  \begin{tabular}{c}
      ...
      (d) 屋外逆方向
  \end{tabular}
  \caption{各走行経路においてのメトリックマップ上でのノードの位置}
...
\begin{table*}[t]
  ...
  \small
  \caption{定量評価：屋内交差}
  \label{table:loop_detection_eval}%\vspace*{-2mm}
  \begin{tabular}{cccc}
  \toprule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  & \textbf{Thresholding} & \textbf{with Panorama Shift} & \textbf{Ours}\\
...
\bibitem{ref:bag-of-words}
  James Philbin, Ondrej Chum, Michael Isard, Josef Sivic, and Andrew Zisserman:
  ``Object Retrieval with Large Vocabularies and Fast Spatial Matching'',
  \textit{Proc. of IEEE Int. Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2007.

\bibitem{ref:VLAD}
  Hervé Jégou, Matthijs Douze, Cordelia Schmid, and Patrick Pérez:
  ``Aggregating Local Descriptors into a Compact Image Representation'',
  \textit{IEEE Computer Society Conf. on Computer Vision and Pattern Recognition(CVPR)}, 2010.
