...
% トポロジカルマップは，環境をノードとアークからなるグラフ構造として抽象的に表現する手法である．
% このような表現は，環境変化に対する頑健性や計算効率の面で優れており，経路計画および自律移動において有効であることが示されている\cite{ref:Metric-Topological}.
% さらに，人間の空間認知においてもトポロジカル構造に基づく認知地図の存在が示唆されており，ロボティクスへの応用が進められている\cite{ref:The-Spatial-Semantic-Hierarchy}．
% 本稿では，これらの利点を活かしつつ，全周パノラマ画像の方位の特性と空間的一貫性チェックに基づくトポロジカルマップの構築手法を提案する．
% \subsection{トポロジカルマップ}
\subsection{Topological Maps}
ロボティクスで用いる地図は，メトリックマップとトポロジカルマップの2種類に大別される．
メトリックマップは環境を座標系上で精密に表現するため高精度な自己位置推定が可能であるが，環境の規模が大きくなるほど経路計画などに要する計算コストやメモリ使用量が増大する．
また，物体が移動・変化し環境が大きく変化する場合はマップの再構築が必要であり，環境変化に対して頑健とは言えない．
一方，トポロジカルマップは環境を特徴的な場所（ノード）とそれらの接続（アーク）によるグラフ構造で抽象的に表現する手法である．
このような表現は，計算効率や環境変化に対する頑健性の面で優れており，経路計画および自律移動において有効であり，人間の空間認知においてもトポロジカル構造に基づく認知地図の存在が示唆され，ロボティクスへの応用が進められている\cite{ref:Metric-Topological,ref:Navigation-in-hybrid-metric-topological-maps}．

% Kuipers ら\cite{ref:The-Spatial-Semantic-Hierarchy}は，認知地図（Cognitive Map）の概念をロボットの空間表現に取り入れ，環境を複数の抽象レベルで捉える Spatial Semantic Hierarchy（SSH）を提案した．

また Thrun \cite{ref:Metric-Topological}は，グリッドマップの幾何学的な精密さと，トポロジカルマップの計算効率の高さを統合したハイブリッドな地図構築手法を提案した．
このメトリック・トポロジカルマップは，詳細な幾何情報と高次の抽象表現を両立させた地図表現を実現している．

Konolige ら\cite{ref:Navigation-in-hybrid-metric-topological-maps}は，メトリック・トポロジカルマップを用いたナビゲーション手法を提案している．
小領域のメトリックな局所地図による局所的なナビゲーションとトポロジカルグラフによる大域的な経路計画を行うことで，詳細な幾何情報に常時依存する必要はないことを示唆している．

\subsection{Visual Place Recognition}
Visual Place Recognition（VPR）は，画像を用いて一度訪れた地点を認識する技術である．
初期の VPR は，SIFT や SURF などの local descriptors を bag of visual words（BoVW）\cite{ref:bag-of-visual-words} や VLAD\cite{ref:VLAD} により global descriptors に集約し，画像検索タスクに拡張する形式が取られていた．
その後，NetVLAD\cite{ref:NetVLAD} や GeM\cite{ref:GeM}，CosPlace\cite{ref:CosPlace}，AnyLoc\cite{ref:AnyLoc} といった深層学習ベースの手法が登場し，高い性能を発揮している．

通常のカメラの画像では，同一地点であってもカメラの向きが異なることで撮影できる景色が異なり，適切に VPR を行うことできない．
そこで，全天球カメラで取得した全周画像を用いて各地点の全ての景色を網羅する画像を取得することで，この問題に対処した\cite{ref:robosym_ogiwara}．
しかし，再訪地点が交差・逆方向の場合においては全周画像を構成する物体の位置関係が異なることで，同一地点であっても VPR による特徴ベクトルに違いが発生し，適切に VPR を行うことできない場合があった．
本稿では，パノラマシフトによる全周画像の切れ目の変更を行うことで上記問題の解決を図る．
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{fig/pipeline_and_concept.pdf} \vspace*{-5mm}
  \caption{提案手法のパイプライン}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{fig/panorama_shift_concept.pdf} \vspace*{-5mm}
  \caption{パノラマシフトの概念図}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{fig/sliding_window_concept.pdf} \vspace*{-5mm}
  \caption{空間的一貫性チェックの概念図}
  \label{fig:sliding_window_concept}
\end{figure*}
...
\subsection{全周画像のパノラマシフトに基づく特徴抽出}
...
式\eqref{eq:feature_vectors}に各画像から抽出された特徴ベクトルの集合を示す．
\begin{align}
  V = \{\bm{v}_1, \bm{v}_2, \ldots, \bm{v}_n\}
  \label{eq:feature_vectors}
\end{align}
ここで，$n$は抽出された特徴ベクトルの総数を表す．

% \subsection{ノードの作成とアークの接続}
\subsection{Node Creation and Adjacent Arc Connection}
...
類似度に対して閾値 \textit{node\_sim\_th} を設定し，ノードの粗密を調整する．
具体的には，特徴ベクトル $\bm{v}_i$ を基準として，それ以降の特徴ベクトル $\bm{v}_j$ ($j>i$) との類似度が \textit{node\_sim\_th} を上回った場合はノードは追加されず，下回った場合は$\bm{v}_j$が新たなノードとして追加される．
次の回では，基準となる特徴ベクトルを $\bm{v}_j$ とし，同様の処理を繰り返すことでロボットの走行経路に沿ったノードとアークが作成される．
% 次の回では，基準となる特徴ベクトルを $\bm{v}_j$ とし，同様の処理を繰り返すことでロボットの走行経路に沿ったノードとアークが作成される．
式\eqref{eq:nodes}に，作成されたノード列を示す．
\begin{align}
  N = \{\bm{n}_1, \bm{n}_2, \ldots, \bm{n}_m\}
  \label{eq:nodes}
\end{align}
ここで，$m$は作成されたノードの総数を表す．

提案手法では，前稿\cite{ref:robosym_ogiwara}よりも \textit{node\_sim\_th} を高く設定し，ノード間の距離を短くする．
...
ウィンドウには事前に決められた \textit{window\_size} 個のノードが格納され，それらとクエリノードとの類似度を計算する．
...
まず，ウィンドウ内のノードのうち，$\bm{n}_q$ とループ検出に対する閾値 \textit{loop\_sim\_th} 以上の類似度をもつ $\bm{n}_t$ をカウントする．
次に，一つのウィンドウに対するカウントがカウント用の閾値 \textit{count\_th} を超えた場合，そのウィンドウをループ検出の候補ウィンドウとする．
...
スキップする範囲は，クエリノード $\bm{n}_q$ の前後 \textit{neighbor\_node\_num} 分のノードとする．
...
\begin{figure}[t]
  ...
  \end{tabular} 
  \begin{tabular}{c}
      \includegraphics[width=0.9\linewidth]{fig/env_dkan_outdoor_inverse.pdf}\\
      (c) 屋外逆方向 \vspace*{2mm}
  \end{tabular} 
...
実験環境および走行経路は，屋内交差，屋内逆方向，屋外交差，屋外逆方向の4種類である．
...
\subsection{画像のパノラマシフトに基づく特徴抽出によるコサイン類似度の変化}
\label{sec:ex_panorama_shift}

% 図\ref{fig:panorama_image_reverse_direction}に屋内環境,図\ref{fig:panorama_image_outdoor_crossroads_08_30}に屋外環境で取得した通常のパノラマ画像とパノラマシフト後の画像の例を示す．
% 図\ref{fig:panorama_image_reverse_direction}，\ref{fig:panorama_image_outdoor_crossroads_08_30}共にパノラマシフトなしの場合は，画像の両端で人が見切れているのに対し，パノラマシフトありの場合は画像の切れ目が変更され，人が中央に配置されていることが分かる．

図\ref{fig:ex_sim_panorama_shift_outdoor}に，屋外環境で取得した画像列に対するコサイン類似度の変化の例を示す．
図\ref{fig:ex_sim_panorama_shift_outdoor}(a)はパノラマシフトなしの場合，(b)はパノラマシフトありの場合である．
Query 画像と Target 画像は，図\ref{fig:env_images}(b)の走行経路において交差地点で取得した画像であり，同一地点ではあるがロボットの進行方向が異なる際に取得した画像である．

図\ref{fig:ex_sim_panorama_shift_outdoor}(a)では，Query 画像と Target 画像を比較すると，画像を構成する建物や自然物は同じであるが，位置関係が異なる．
一方，図\ref{fig:ex_sim_panorama_shift_outdoor}(b)では，パノラマシフトにより画像の切れ目が変更され，人などの動くものを除き，画像を構成する物体の位置関係が同じになっている．
コサイン類似度はパノラマシフトなしの場合は0.360であるのに対し，パノラマシフトありの場合は0.641であり，78.1\%の改善が見られる．
ただし，コサイン類似度の有効数字は3桁としている．
これらの結果から，同一地点を異なる方向から再訪した場合でも，パノラマシフトにより画像の切れ目を変更し，画像内の物体の位置関係を揃えることで，視覚的類似度を向上させることができると考えられる．


% \begin{figure}[t]
%   \centering
%   \begin{tabular}{cc}
%   \includegraphics[width=0.5\linewidth]{fig/panorama_image_reverse_direction.pdf} & \includegraphics[width=0.45\linewidth]{fig/panorama_image_reverse_direction_with_shift.pdf} \vspace*{1mm} \\
%   (a) パノラマシフトなし & (b) パノラマシフトあり
%   \end{tabular}  
%   \caption{屋内環境のパノラマ画像の例}
%   \label{fig:panorama_image_reverse_direction}
% \end{figure}

% \begin{figure}[t]
%   \centering
%   \begin{tabular}{cc}
%   \includegraphics[width=0.5\linewidth]{fig/panorama_image_outdoor_crossroads_08_30.pdf} & \includegraphics[width=0.45\linewidth]{fig/panorama_image_outdoor_crossroads_08_30_with_shift.pdf} \vspace*{1mm} \\
%   (a) パノラマシフトなし & (b) パノラマシフトあり
%   \end{tabular}  
%   \caption{屋外環境のパノラマ画像の例}
%   \label{fig:panorama_image_outdoor_crossroads_08_30}
% \end{figure}
...
\begin{figure}[t]
  \centering
  \begin{tabular}{c}
      \includegraphics[width=\linewidth]{fig/ex_sim_panorama_shift_off.pdf}\\
      (a) パノラマシフトなし
      \label{fig:ex_sim_panorama_shift_off}
  \end{tabular} 
  \begin{tabular}{c}
      \includegraphics[width=\linewidth]{fig/ex_sim_panorama_shift_on.pdf}\\
      (b) パノラマシフトあり
      \label{fig:ex_sim_panorama_shift_on}
  \end{tabular} 
  \caption{パノラマシフトによるコサイン類似度の変化}
  \label{fig:ex_sim_panorama_shift_outdoor}
\end{figure}


% \subsection{トポロジカルマップの構築}
\subsection{空間的一貫性チェックを用いたループ検出によるトポロジカルマップの構築}

本稿のトポロジカルマップの構築では，動画から画像列を作成する際の一定の時間間隔を1.0 [s]，ノードの粗密を調整するパラメータ \textit{node\_sim\_th} = 0.80 で統一した．

図\ref{fig:ex_add_node}に各走行経路においてのメトリックマップ上でのノードの作成位置を示す．
ノードの数や粗密は各画像から抽出される特徴ベクトルのコサイン類似度に依存するため，同じ動画データを用いてもパノラマシフトの有無によりノードの数や位置が異なる．
図\ref{fig:ex_add_node}には，パノラマシフトを行った後の画像を用いて作成したノードを示す．
\ref{sec:exp_conditions}節より，ノードの位置はメトリックな情報から算出している．
図\ref{fig:ex_add_node}より，\textit{node\_sim\_th} を高い値に設定したことで，ノード間隔は短くなり，図\ref{fig:env_images}に示す走行経路と比較すると，走行経路上のほぼ全ての地点にノードが作成されていることが分かる．

次に，構築したトポロジカルマップと，ループ検出の正誤を評価する．
ループ検出の正誤判定には，\ref{sec:exp_conditions}節で述べたメトリックな情報を用いた．
本稿では，ループ検出の精度評価のために3つの手法でトポロジカルマップを構築，それぞれの結果を比較し，定性・定量評価する．

\textbf{単純な閾値}：通常の全周画像を用いて特徴ベクトルを抽出し，クエリノードとターゲットノードのコサイン類似度が閾値 \textit{loop\_sim\_th} を上回った場合にループ検出を行う．
...
全ての実験において，\textit{loop\_sim\_th} = 0.55 に統一した．
パノラマシフト + 一貫性チェックにおいては，\textit{window\_size} = 5，\textit{count\_th} = 2に統一し，\textit{neighbor\_node\_num} は走行経路により適応的に変更した．
...
ループ検出の正誤判定に用いるメトリック距離の閾値は，1.0 [m] とした．

図\ref{fig:ex_ld_dkanjuuji3}〜\ref{fig:ex_ld_outdoor_inverse}に，各走行経路において構築したトポロジカルマップを示す．
...
次に，屋外の走行経路である図\ref{fig:env_images}(b)，\ref{fig:env_images}(c)とその実験結果である図\ref{fig:ex_ld_outdoor_crossroads}，\ref{fig:ex_ld_outdoor_inverse}を比較する．
% 次に，屋外環境である図\ref{fig:ex_ld_outdoor_crossroads}と，図\ref{fig:env_images}(b)の走行経路を比較する．
まず屋外交差において，単純な閾値では交差地点をループ検出できていないのに対し，パノラマシフト + 単純な閾値では交差地点をループ検出でき,大きなループを構築できている．
...
\begin{figure}[t]
  \centering
  \begin{tabular}{c}
      \includegraphics[width=0.9\linewidth]{fig/ex_add_node_dkanjuuji3.pdf}\\
      (a) 屋内交差
  \end{tabular} 
  \begin{tabular}{c}
      \includegraphics[width=0.9\linewidth]{fig/ex_add_node_reverse_direction.pdf}\\
      (b) 屋内逆方向
  \end{tabular}
  \begin{tabular}{c}
      \includegraphics[width=0.9\linewidth]{fig/ex_add_node_outdoor_crossroads.pdf}\\
      (c) 屋外交差
  \end{tabular}
  \begin{tabular}{c}
      \includegraphics[width=0.9\linewidth]{fig/ex_add_node_outdoor_invverse.pdf}\\
      (d) 屋外逆方向
  \end{tabular}
  \caption{メトリックマップ上でのノードが作成された位置}
  \label{fig:ex_add_node}
\end{figure}
...
\begin{figure*}
  ...
  \begin{tabular}{ccc}
    ...
    \includegraphics[width=0.45\linewidth]{fig/ex_ld_outdoor_inverse_spatial.pdf} \\
    (a) 単純な閾値 & (b) パノラマシフト + 単純な閾値 & (c) パノラマシフト + 一貫性チェック (Ours) \\
  \end{tabular}
...
\begin{table}[t]
  ...
  \small
  \caption{手法一覧}
  \label{table:methods}%\vspace*{-2mm}
  \begin{tabular}{cccc}
  \toprule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  \textbf{Function} & \textbf{Simple Th.} & \textbf{P. Shift} & \textbf{Ours}\\
  \midrule%[0.05em]  % デフォルト 0.05em（\lightrulewidth）
  Panorama Shift & \Cross & \Circle & \Circle \\
  Consistency Check & \Cross & \Cross & \Circle \\
...
\begin{table}[t]
  ...
  \small
  \caption{定量評価：屋内交差}
  \label{table:ld_eval_indoor_cross}%\vspace*{-2mm}
  \begin{tabular}{cccc}
  \toprule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  & \textbf{Simple Th.} & \textbf{P. Shift} & \textbf{Ours}\\
  \midrule%[0.05em]  % デフォルト 0.05em（\lightrulewidth）
  TP & 14 & 18 & 9\\
  FP & 16 & 34 & 0\\
  TN & 72 & 58 & 81\\
  FN & 2 & 0 & 9 \\
  Precision \textbf{$\uparrow$} & 47\% & 35\% & \textbf{100\%}\\
  Accuracy \textbf{$\uparrow$} & 83\% & 69\% & \textbf{91\%}\\
  F1 Score \textbf{$\uparrow$} & 61\% & 51\% & \textbf{67\%}\\
  \bottomrule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \small
  \caption{定量評価：屋内逆方向}
  \label{table:ld_eval_indoor_reverse}%\vspace*{-2mm}
  \begin{tabular}{cccc}
  \toprule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  & \textbf{Simple Th.} & \textbf{P. Shift \& Simple Th.} & \textbf{Ours}\\
  \midrule%[0.05em]  % デフォルト 0.05em（\lightrulewidth）
  TP & 49 & 87 & 34\\
  FP & 146 & 125 & 6\\
  TN & 64 & 66 & 105\\
  FN & 49 & 9 &  62\\
  Precision \textbf{$\uparrow$} & 25\% & 41\% & \textbf{85\%}\\
  Accuracy \textbf{$\uparrow$} & 37\% & 53\% & \textbf{67\%}\\
  F1 Score \textbf{$\uparrow$} & 33\% & \textbf{56\%} & 50\%\\
  \bottomrule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \small
  \caption{定量評価：屋外交差}
  \label{table:ld_eval_outdoor_cross}%\vspace*{-2mm}
  \begin{tabular}{cccc}
  \toprule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  & \textbf{Simple Th.} & \textbf{P. Shift \& Simple Th.} & \textbf{Ours}\\
  \midrule%[0.05em]  % デフォルト 0.05em（\lightrulewidth）
  ...
  TN & 4 & 7 & 314\\
  FN & 4 & 2 & 2\\
  Precision \textbf{$\uparrow$} & 0\% & 0.28\% & \textbf{67\%}\\
  Accuracy \textbf{$\uparrow$} & 0.48\% & 1.2\% & \textbf{99\%}\\
  F1 Score \textbf{$\uparrow$} & 0\% & 0.55\% & \textbf{57\%}\\
  \bottomrule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \small
  \caption{定量評価：屋外逆方向}
  \label{table:ld_eval_outdoor_reverse}%\vspace*{-2mm}
  \begin{tabular}{cccc}
  \toprule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  & \textbf{Simple Th.} & \textbf{P. Shift \& Simple Th.} & \textbf{Ours}\\
  \midrule%[0.05em]  % デフォルト 0.05em（\lightrulewidth）
  ...
  TN & 1 & 16 & 317\\
  FN & 50 & 15 & 42\\
  Precision \textbf{$\uparrow$} & 2.7\% & 5.8\% & \textbf{44\%}\\
  Accuracy \textbf{$\uparrow$} & 2.6\% & 6.9\% & \textbf{79\%}\\
  F1 Score \textbf{$\uparrow$} & 5.0\% & 11\% & \textbf{47\%}\\
  \bottomrule%[0.08em]  % デフォルト 0.08em（\heavyrulewidth）
  \end{tabular}
\end{table}
...
\bibitem{ref:robosym_ogiwara}
  Daichi Ogiwara, Naoki Takahashi, Kosuke Kimizuka, Takaya Nakao, Yoshitaka Hara, and Yoji Kuroda: 
  ``Topological Mapping using Visual Place Recognition'',
  \textit{Proc. of Robotics Symposia (ROBOSYM)}, 2025. (in Japanese)
...
\bibitem{ref:The-Spatial-Semantic-Hierarchy}
  ...
  \textit{Artiﬁcial Intelligence}, vol. 119, no. 1--2, pp. 191--233, 2000.

\bibitem{ref:Metric-Topological}
  Sebastian Thrun:
  ``Learning Metric-Topological Maps for Indoor Mobile Robot Navigation'',
  \textit{Artiﬁcial Intelligence}, vol. 99, no. 1, pp. 21--71, 1998.

\bibitem{ref:Navigation-in-hybrid-metric-topological-maps}
  Kurt Konolige, Eitan Marder-Eppstein, and Bhaskara Marthi:
  ``Navigation in Hybrid Metric-Topological Maps'',
  \textit{Proc. of IEEE Int. Conf. on Robotics and Automation (ICRA)}, 2011.

\bibitem{ref:bag-of-visual-words}
  James Philbin, Ondrej Chum, Michael Isard, Josef Sivic, and Andrew Zisserman:
  ``Object Retrieval with Large Vocabularies and Fast Spatial Matching'',
  \textit{Proc. of IEEE/CVF Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2007.
  % IEEE Int. Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2007.

\bibitem{ref:VLAD}
  Hervé Jégou, Matthijs Douze, Cordelia Schmid, and Patrick Pérez:
  ``Aggregating Local Descriptors into a Compact Image Representation'',
  \textit{Proc. of IEEE/CVF Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2010.
...
\bibitem{ref:NetVLAD}
  ...
  \textit{Proc. of IEEE/CVF Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2016.

\bibitem{ref:GeM}
  Filip Radenović, Giorgos Tolias, and Ondrej Chum:
  ``Fine-Tuning CNN Image Retrieval with No Human Annotation'',
  \textit{IEEE Trans. on Pattern Analysis and Machine Intelligence}, vol. 41, no. 7, pp. 1655--1668, 2019.

\bibitem{ref:CosPlace}
  Gabriele Berton, Carlo Masone, and Barbara Caputo:
  ``Rethinking Visual Geo-localization for Large-Scale Applications'',
  % \textit{IEEE/CVF Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2022.
  \textit{Proc. of IEEE/CVF Conf. on Computer Vision and Pattern Recognition (CVPR)}, 2022.
